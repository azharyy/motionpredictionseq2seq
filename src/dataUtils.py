...
def revert_output_format(poses, mean, std, ignore_dim, actions, one_hot):
  """
  Transforms the neural network's output into a more user-friendly format.

  Args
    poses: Output from the TensorFlow model. A list of (seq_length) entries,
    each with a (batch_size, dim) output.
  Returns
    transformed_poses: A tensor of size (batch_size, seq_length, dim). Each
    batch is a sequence of n-by-d poses.
  """
  ...
  # Reshape and transpose datat
  transforme_poses = np.transpose(
      np.reshape(
  ...
  transformed_poses = []
  for i in range(transforme_poses.shape[0]):
    transformed_poses.append(
      unNormalizeData(transforme_poses[i, :, :], mean, std, ignore_dim, actions, one_hot))
