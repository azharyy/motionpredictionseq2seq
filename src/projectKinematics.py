...
def main():
  ...
  with h5py.File( 'samples.h5', 'r' ) as h5f:
    expmap_gt = h5f['expmap/gt/walking_0'][:]                 #<--------change the action displayed by changing after the "gt/"
    expmap_pred = h5f['expmap/preds/walking_0'][:]            #<--------change the action displayed by changing after the "gt/"
